...
- The 3D scene lives in `src/components/Scene.tsx`; archive-vessel geometry/animation is in `src/components/ArchiveVessel.tsx`; chamber, orbit logos, and generated logo textures are in `src/components/Studio.tsx`.
...
- Avoid reintroducing generic glass cards or large floating HUD panels; current direction is an antiquarian archive-vessel artifact with an instrument-spine nav and compact specimen plate.
